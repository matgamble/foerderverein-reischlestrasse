...
## Sprachauswahl (Google-Übersetzungs-Widget)
- Im `.topbar` sitzt `#google_translate_element` (Google Website Translator, `translate_a/element.js`), eingebunden für Türkisch, Russisch, Ukrainisch, Französisch, Spanisch, Portugiesisch, Polnisch (`includedLanguages` in der Init-Funktion am Seitenende). Bewusst diese Lösung statt Hand-Übersetzung gewählt (Absprache mit der Nutzerin) – übersetzt automatisch auch künftige, noch nicht geschriebene Inhalte, Qualität ist maschinell/nicht perfekt.
- CSS blendet nur die aufdringliche Google-Top-Leiste aus (`.goog-te-banner-frame`) und verhindert den Seiten-Versatz (`body { top: 0 !important }`) – das Dropdown selbst (inkl. Google-Attribution) bleibt sichtbar, wie von Google verlangt.
- **Konnte in dieser Umgebung nicht visuell getestet werden**: `translate.google.com` ist über den lokalen Proxy blockiert (siehe Netzwerk-Einschränkungen unten) – das betrifft nur diese Entwicklungsumgebung, nicht echte Website-Besucher:innen auf GitHub Pages, die laden das Skript ganz normal in ihrem eigenen Browser. Nach dem nächsten Deploy auf der echten URL prüfen, ob das Dropdown wie erwartet erscheint und funktioniert.
- Datenschutzerklärung wurde entsprechend ergänzt (Cookie-Hinweis, Verweis auf Googles Datenschutzerklärung) – das war vorher fälschlich als "keine Cookies/Tracking" formuliert.
