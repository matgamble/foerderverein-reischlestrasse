...
## Sprachauswahl (Flaggen-Dropdown, Link zu Google Übersetzer)
- **Verlauf**: Erste Version band das eingebettete "Google Website Translator"-Widget ein (`#google_translate_element`, per JS ferngesteuert über das versteckte `.goog-te-combo`-Select). Das ist aber ein von Google seit Jahren inoffiziell/unsupported geführtes Legacy-Feature (2018 offiziell eingestellt, seit 2020 nur noch informell für gemeinnützige Seiten geduldet) und lief bei der Nutzerin auf dem echten Handy nicht zuverlässig (u. a. fehlten neu ergänzte Sprachcodes in `includedLanguages` – ein konkreter Bug, der behoben wurde, aber die grundsätzliche Fragilität blieb). Daher **umgestellt auf eine robustere Lösung**.
- **Aktueller Stand**: Im `.topbar` sitzt ein Flaggen-Dropdown (`.lang-picker`, `<details>`-Muster wie `.mobile-menu`). Jede Sprache außer Deutsch ist ein `<a target="_blank">`-Link, dessen `href` per `lang.js` beim Laden auf `https://translate.google.com/translate?sl=de&tl=<code>&u=<aktuelle-Seiten-URL>` gesetzt wird – öffnet die von Google übersetzte Version der Seite in einem neuen Tab. Das ist Googles aktiv genutzter URL-Übersetzungsdienst (nicht das abgekündigte Widget), dadurch deutlich stabiler. Sprachen: Türkisch, Russisch, Ukrainisch, Französisch, Spanisch, Portugiesisch, Polnisch, Albanisch, Englisch, Kurdisch (Kurdistan-Flagge als Inline-SVG, da kein Unicode-Ländercode existiert).
- Kein Google-Skript mehr auf der eigenen Seite eingebunden, kein Cookie durch uns – Datenschutzerklärung entsprechend angepasst (Klick auf eine Sprache verlässt die eigene Seite Richtung Google, keine Einbindung mehr vorher).
- Klick auf die aktuell gewählte Flagge (rundes Icon) aktualisiert nur die Anzeige (letzte Auswahl als Erinnerung), da die eigentliche Übersetzung im neuen Tab passiert und die Ursprungsseite unverändert Deutsch bleibt.
